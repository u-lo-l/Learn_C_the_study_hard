...
void Database_set(Connection *conn,
					int id,
					const char *name,
					const char *email)
{
	...
	addr->set = 1;
	/* strncpy part has some problem if 3rd parameter exceed MAX_DATA : 512
	 * It can be handled by adding null char at the end of the string
	 * or change 3rd parameter to 512 if it is bigger than 512
	 */
...
int main (int argc, char *argv[])
{
	...
	switch(action)
	{
		...
		case 'l' : 
			...
			break;
		default :
			die("Invalid action : 'c'reation, 'g'et, 's'et, 'd'elete, 'l'ist");
	}
	Database_close(conn);
